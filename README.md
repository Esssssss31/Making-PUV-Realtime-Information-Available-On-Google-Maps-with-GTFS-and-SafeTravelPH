...
It involves creating a GTFS (Google Transit Feed Specifications) feed, a standardized format for public transport from Google, from the realtime data generated by the SafeTravelPH mobile app, a free app for drivers/operators to track their realtime location, altitude occupancy, and boarding/alighting. SafeTravelPH wants to promote the use of GTFS in PUVs as part of the modernization program of the government. 
...
Some of the benefits according to research for both riders/commuters and transport operators/agencies/consortia are:

### Riders/Commuters:
1. LOWER WAIT TIMES. Because PUV riders can time their walk or commute to the transit stops with when the PUV is actually arriving.

2. LOWER TRAVEL TIMES. See benefit #1, but adding that riders can see realtime info about the location of PUVs they can look at different options that might be arriving sooner than others, adapting on-the-go.

3. GREATER PEACE OF MIND. Sometimes with the published schedule of operators you don't know if the PUV is actually going to arrive but with that real-time you have better confirmation.

### PUV Operators (and Regulators)
1. VALUABLE DATA. PUV providers and the government can collect this great amount of data and make reporting, monitoring, and evaluation easier. 

2. GREATER RIDERSHIP AND CUSTOMER SATISFACTION. With more reliable and accessible public transport system through information provision, more people should shift to public transport from using private cars and TNCs.

3. RETURN ON INVESTMENT. Increasing ridership is essential for sustaining the operations of public transport, especially with the investments being made under the PUV modernization program. 

# Publishing Public Transport Feeds Using GTFS from SafeTravelPH Drivers App to Google Maps

This is an open source project for publishing a General Transit Feed Specification (GTFS) feed for public transport system in the Philippines. The project consists of Python scripts for ingesting, processing, and storing real-time transit data, as well as generating a GTFS feed that can be published on Google Maps and other transit mapping platforms. 

This project of publishing GTFS feeds to Google Maps uses data from a PUV Driver's tracking mobile app via MQTT and Apache Kafka. The mobile app in this case is the [SafeTravelPH app](https://play.google.com/store/apps/details?id=ph.safetravel.app&hl=en&gl=US). A case study of this data system was done in partnership with Puerto Princesa City's ISTOPP (Information System for Transport Operations in Puerto Princesa) under the [NEDA Innovation Grant in 2022](https://www.gmanetwork.com/news/money/economy/861554/innovation-council-approved-19-projects-worth-p115m-neda-chief/story/).
